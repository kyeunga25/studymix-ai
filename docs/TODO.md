...
- [x] 共用有界音訊結構檢查會在 metadata-valid 選檔後立即以 `File.slice()` 執行，換檔／離頁會中止舊讀取，成功後以雙語顯示辨識格式並解鎖後續控制；提交上載前會再獨立核對一次，確認時則以固定 `HEAD` ETag 的 R2 range reads，分別辨識目前供應商接受的 MP3 frame、RIFF/WAVE chunks、M4A audio track、AAC ADTS frames 及 Ogg audio identification packet，全程不載入完整大檔。改副檔名的非音訊會在零法律接受 mutation、零 upload metadata／零 bytes 外送下收到雙語可及錯誤；繞過 browser 的 MIME／container mismatch 會在 server fail closed 並走 owner-scoped cleanup，暫時讀不到固定版本則保持 pending／retryable 而不誤刪。Unit、Worker ownership／並發／R2 failure 及合成 Chromium 反例均不使用真實音訊或外部 AI。此檢查只確認結構，不聲稱檔案一定是歌曲、可完整解碼、品質合格或已具權利。
...
- [x] 匿名首頁只同步載入公開 overview；登入、公開法律及私人應用使用具雙語載入狀態及安全重試的獨立 lazy chunks。production 入口 JS 由 368.53 kB 降至 282.01 kB（gzip 114.30 kB 降至 86.62 kB），精確 route tests 會把未知及 `/app` lookalike 路徑留在公開首頁，Playwright 亦覆蓋 deferred chunk failure。
- [x] CSS 跟隨 route lazy loading：production 匿名入口 CSS 由 46.74 kB 降至 15.53 kB（gzip 10.04 kB 降至 4.03 kB），登入 10.28 kB 及私人／法律 26.53 kB 樣式按需載入；四類頁面及 390×844 瀏覽器均無水平溢出或 console error。
- [x] 公開法律正文、私人 workspace runtime、工作區準備狀態、最近工作紀錄與工作結果體驗已分開：原 81.70 kB deferred chunk 改為 32.68 kB public-legal、48.85 kB private-app（gzip 15.49 kB）、session 成功後才載入的 2.88 kB access-readiness（gzip 1.25 kB）與 5.35 kB recent-job-history（gzip 2.25 kB），以及只有 job／job error 出現時才載入的 15.46 kB job-experience（gzip 5.58 kB）；另有 8.96 kB（gzip 2.96 kB）共用 job API、metadata-valid 選檔後才載入的 6.29 kB（gzip 2.37 kB）音訊結構檢查器，以及按需載入的 1.54 kB legal-acceptance、1.65 kB local-AI helpers、0.62 kB private-API helper 及 2.97 kB 共用雙語 site chrome。Chromium 以延遲 chunk 證明初始 workspace 不會要求工作體驗；選檔成功與假副檔名反例則證明檢查器只在本機預檢需要時載入，並保留可存取雙語狀態及原有結果流程。
...
- [x] Production Vite build 以 raw／gzip byte budgets fail closed：匿名入口、登入、公開法律、私人 app、私人工作區準備狀態、私人最近工作紀錄、私人工作體驗、入口／總 CSS、總 JS 及背景 WebP 必須齊全且不超標；合成 unit tests 覆蓋正常、缺失、重複與超標輸出，錯誤不公開本機 module path。
