...
- [x] CSS 跟隨 route lazy loading：production 匿名入口 CSS 由 46.74 kB 降至 15.53 kB（gzip 10.04 kB 降至 4.03 kB），登入 10.28 kB 及私人／法律 22.54 kB 樣式按需載入；四類頁面及 390×844 瀏覽器均無水平溢出或 console error。
- [x] 公開法律正文、私人 workspace runtime 與工作結果體驗已分開：原 81.70 kB deferred chunk 改為 32.68 kB public-legal、56.85 kB private-app（gzip 17.68 kB）及只有 job／job error 出現時才載入的 15.46 kB job-experience（gzip 5.58 kB）；另有 metadata-valid 選檔後才載入的 6.29 kB（gzip 2.37 kB）共用音訊結構檢查器，以及按需載入的 1.54 kB legal-acceptance、1.65 kB local-AI helpers、0.62 kB private-API helper 及 2.97 kB 共用雙語 site chrome。Chromium 以延遲 chunk 證明初始 workspace 不會要求工作體驗；選檔成功與假副檔名反例則證明檢查器只在本機預檢需要時載入，並保留可存取雙語狀態及原有結果流程。
...
- [x] Web create success 必須重新綁定已驗證的 upload ID、preset ID／version 及候選數；poll／cancel／delete 及 output-download 成功回應亦必須識別所要求的 job／output ID，錯配不會開始追蹤、轉換狀態或清空目前結果。所有 path 在組合前重做 Zod opaque ID 驗證，錯 prefix、長度、path／query suffix 會本機 non-retryable fail closed 且零 fetch；62 項 job client tests 與兩個 Chromium mismatch 反例同時保留 caller Abort、timeout、bounded response、idempotent create 及獨立 Worker owner 驗證。
